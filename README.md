...
This source tree also includes `mcdf-registry-server`, a local-testable central server for MCDF archival.

The desktop app can upload an MCDF to the server from Settings. The server extracts the package into full internal files, stores those files as the durable archive unit, writes a rebuild manifest, and can optionally publish those artifacts to GHCR through ORAS.

Local test:

```bash
cd src-tauri
cargo run --bin mcdf-registry-server -- \
  --bind 127.0.0.1:8080 \
  --public-url http://127.0.0.1:8080 \
  --data-dir ../.local-mcdf-registry-test \
  --storage-mode local
```

End-to-end test with a real MCDF:
...
mcdf-registry-server \
  ...
  --server-auth-token "$MCDF_SERVER_AUTH_TOKEN" \
  --ghcr-owner thebigtree \
  --ghcr-package-prefix mcdf \
  --ghcr-username "$MCDF_GHCR_USERNAME" \
  --ghcr-token "$MCDF_GHCR_TOKEN"
```

See `docs/CENTRAL_REGISTRY_SERVER.adoc` for the API and deployment details.
